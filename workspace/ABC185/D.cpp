...
int main() {
  ...
  for (int i = 0; i < m; i++) {
    ll tmp; 
    if (i == 0 && a[i] >= 1) step.push_back(a[i]);
    if (i == m-1) {
      if (n - 1 - a[i] >= 1) step.push_back(n - 1 - a[i]);
      continue; 
    }
    tmp = a[i+1] - a[i] - 1;
    if (tmp >= 1) step.push_back(tmp);
  }

  ll stamp = 1000000000;
  for (auto x : step) stamp = min(stamp, x);

  ll ans = 0;
  for (auto x : step) {
    ans += (x % stamp == 0 ? x / stamp : x / stamp + 1);
  }
