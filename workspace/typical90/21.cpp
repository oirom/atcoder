...
ll N, M, A[1<<18], B[1<<18], cnts;
vector<ll> G[1<<18], H[1<<18], I;
bool chk1[1<<18], chk2[1<<18];

void dfs1(ll pos) {
  chk1[pos] = true;
  for (auto to : G[pos]) {
    if (chk1[to] == false) dfs1(to);
  }
  I.push_back(pos);
}
...
int main() {
  ...
  for (int i=1;i<=N;i++) {
    if (chk1[i] == false) dfs1(i);
  }

  ll Answer = 0;
  reverse(I.begin(), I.end());
  for (int i=1;i<=N;i++) chk1[i] = false;
  for (int i : I) {
    if (chk1[i] == true) continue;
    cnts = 0;
    dfs2(i);
