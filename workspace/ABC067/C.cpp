...
int main() {
  ...
  ll n, sum = 0;
  cin >> n;
  vector<ll> a(n), aacc(n);
  for (int i = 0; i < n; i++) { 
    cin >> a[i];
    sum += a[i];
    if (i == 0) aacc[i] = a[i];
    else aacc[i] = aacc[i-1] + a[i];
  }
  //cout << sum << endl;
  //for (auto x : aacc) cout << x << endl;

  ll diff = 3000000000, ans2 = -1;
  for (int i = 0; i < n-1; i++) {
    if (diff > abs( (sum - aacc[i]) - aacc[i] )) {
      diff = abs( (sum - aacc[i]) - aacc[i] );
    }
  }
